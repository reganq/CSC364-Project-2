...
- Root cause: The `len` field input for `decode_record` is never checked, so it is possible that memory that is too small has been passed. For example, the crashing bug has a memory segment that is only one byte, meaning `memcpy` fails when copying from the buffer since the memory wasn't allocated.
